...
const InfoCard = ({ img, alt_text, first_text, second_text }) => {
	return (
		<div className="flex items-center pl-3 w-60 h-[74px] border border-solid rounded-[20px] border-[#2C2D47]/[1] bg-gradient-to-r from-[#1B1C36]/[.16] to-[#70717A]/[.16]">
			...
			<div className="ml-3.5 text-[#AFB3CA]">
				<p className="font-medium">{first_text}</p>
				<p className="font-medium">{second_text}</p>
			</div>
		</div>
	);
};
